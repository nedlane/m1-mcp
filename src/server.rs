...
impl M1Server {
    ...
    #[tool(
        description = "Type-check M1 script (inline `source` or a file `path`), returning source/project-scoped diagnostics with paths, exact position/byte ranges, project subjects, and related declaration locations. Pass `project` (a Project.m1prj) to enable cross-script and reference-keyword checks and receive a load report naming skipped auxiliary inputs."
    )]
